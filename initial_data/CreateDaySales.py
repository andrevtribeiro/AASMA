...
def getSales(date,item,events):
    global clients
    sales=0
    p=1
    for i,j in zip(items[item],events):
        if j==1:
            p*=i
    if p<0:
        p=0
    ...
    for client in range(int(clients*p)):
        ...
        if quantity<10:
            quantity=10
        sales+=quantity
        data[date][item].append({
            'client_id': int(np.random.rand()*clients),
            'quantity': quantity
...
for y in range(years):
    ...
    for m in range(len(months_days)):
        for d in range(months_days[m]):
            ...
            date=("%.2d/%.2d/%.2d" %(d+1,m+1,year+y))
            data[date]={}
            data[date]['events']=events
            for i in range(len(items)):
                data[date][i]=[]   
                sales_list[i]+=[getSales(date,i,events)]
                
            
with open('daySales.txt', 'w') as outfile:
